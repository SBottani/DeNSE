...
#    ds.plot.plot_neurons(show=True)
    ds.plot.plot_neurons(axis=ax,show=False)
    fig.savefig("Seeded-neurons", dpi=300)
    plt.close(fig)
...
#    fig2, ax2 = plt.subplots() # pour le graphe de densité
    #ds.plot.plot_neurons(show_density=False, dstep=4., dmax=10, cmap="jet",
    #                     show_neuron_id=True)
    ax1,ax2=ds.plot.plot_neurons(culture=culture,
                         soma_alpha=0.4,
                         axon_color='g',
                         gc_color="r",
                         #axis=ax,
                         ...
                         #dmax=10,
                         show=False)
    fig1=Axis.get_figure(ax1)
    fig2=Axis.get_figure(ax2)
    plt.tight_layout()
    ...
    ax2.grid(False)
#    plt.show()
    fig1.savefig(f"Morphology.pdf", dpi=300)
    fig2.savefig(f"Density.pdf", dpi=300)
    plt.close(fig)
    plt.close(fig2)
    print("plot done")
    print("All done")
